...
export default function UpdatesDrawer({ isOpen, onClose }: UpdatesDrawerProps) {
  ...
  return createPortal(
    <div className="fixed inset-0 z-[9999] flex justify-end">
      ...
      {/* Drawer Panel */}
      <div className="relative w-full sm:w-[400px] h-[100dvh] bg-white dark:bg-slate-900 shadow-2xl flex flex-col animate-in slide-in-from-bottom-full sm:slide-in-from-right-full duration-300 border-l border-slate-100 dark:border-slate-800">
        
        {/* Header */}
        <div className="flex items-center justify-between p-4 sm:p-6 border-b border-slate-100 dark:border-slate-800 shrink-0 bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl z-10">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-full bg-emerald-100 dark:bg-emerald-500/15 flex items-center justify-center border border-emerald-200 dark:border-emerald-500/30">
              <Rocket className="w-5 h-5 text-emerald-600 dark:text-emerald-400" />
            </div>
            <div>
              <h2 className="text-xl font-black text-slate-800 dark:text-slate-100 leading-tight">What&apos;s New</h2>
              <p className="text-xs font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest">PinQuo Changelog</p>
            </div>
          </div>
          <button 
            onClick={onClose} 
            className="p-2.5 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 active:scale-95 text-slate-500 dark:text-slate-300 rounded-full transition-all"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-10 no-scrollbar pb-24">
          ...
          <section>
            <div className="flex items-center gap-2 mb-6">
              ...
              <h3 className="text-sm font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest">Live Now</h3>
            </div>

            {/* Timeline Line */}
            <div className="space-y-8 relative before:absolute before:inset-0 before:ml-[11px] before:-translate-x-px before:h-full before:w-0.5 before:bg-gradient-to-b before:from-emerald-200 before:via-slate-200 before:to-transparent dark:before:from-emerald-500/40 dark:before:via-slate-700 dark:before:to-transparent">
          ...
          <section>
            <div className="flex items-center gap-2 mb-6">
              <Hammer className="w-5 h-5 text-blue-500 dark:text-blue-400" />
              <h3 className="text-sm font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest">In The Pipeline</h3>
            </div>
        ...
        {/* Footer */}
        <div className="p-4 sm:p-6 border-t border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-900 mt-auto shrink-0 pb-[max(1rem,env(safe-area-inset-bottom))]">
          <p className="text-[10px] font-black text-center text-slate-400 dark:text-slate-500 uppercase tracking-widest">
            PinQuo v1.2
          </p>
        </div>
