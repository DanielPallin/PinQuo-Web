...
function PreviewQuoteForm() {
  ...
  const customName = searchParams.get('customName') // NEW: Catching the custom name!
  ...
  // NEW: Include customName in the fallback chain
  ...
  const bgStyle = bgType === 'template' ? templateGradient : 'from-slate-400 to-slate-700'
    ...
    const handlePublish = async () => {
      ...
      // NEW: Included custom_author_name in the database payload
      const { error: dbError } = await supabase
        ...
        .insert({
          ...
          custom_author_name: customName || null, 
          content: quoteText,
  ...
  return (
    <div className="flex flex-col pt-10 px-6 w-full max-w-2xl mx-auto min-h-[calc(100vh-120px)] pb-10">
      ...
      <div className="relative text-center mb-8 shrink-0">
        <button onClick={() => router.back()} title="Go Back" className="absolute left-0 top-0 p-4 hover:bg-slate-100 rounded-full transition">
      ...
      <div className="flex-1 flex flex-col items-center justify-center w-full">
        ...
        <div className="w-full max-w-[460px] bg-white rounded-[32px] shadow-2xl overflow-hidden flex flex-col border border-slate-100 mb-10">
          <div className="relative w-full h-64 shrink-0 pointer-events-none">
            ...
            {bgType === 'avatar' && (
              targetAvatarUrl ? (
                <img src={targetAvatarUrl} alt="" className="absolute inset-0 w-full h-full object-cover mix-blend-overlay opacity-80" />
              ) : (
