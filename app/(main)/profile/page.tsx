...
export default function ProfilePage() {
  ...
  const renderMiniGrid = (quotes: MiniQuote[], onClick: () => void) => {
    ...
    return (
      <button onClick={onClick} className="w-full aspect-square bg-slate-100 rounded-[32px] p-3.5 grid grid-cols-2 grid-rows-2 gap-3 hover:scale-105 active:scale-95 transition-all shadow-inner border-[3px] border-slate-50">
        {slots.map((q, i) => {
    ...
    <div className="flex flex-col w-full max-w-2xl mx-auto min-h-screen bg-white pb-24 pt-6 px-6">
            ...
            {isEditingBio ? (
              <div className="flex flex-col h-full gap-2">
                <textarea 
                  ...
                  className="w-full h-24 bg-transparent text-slate-700 font-medium leading-snug outline-none resize-none"
      ...
      <div className="flex gap-6">
        ...
        <div className="flex-1 flex flex-col gap-3">
          <button className="flex items-center justify-between bg-white p-4 rounded-3xl hover:bg-slate-50 active:bg-slate-100 transition group border border-slate-100 shadow-sm">
          ...
          <button className="flex items-center justify-between bg-white p-4 rounded-3xl hover:bg-slate-50 active:bg-slate-100 transition group border border-slate-100 shadow-sm">
            <div className="flex items-center gap-4">
              ...
              <span className="font-bold text-slate-700 text-lg">Account settings</span>
            </div>
            <ChevronRight className="w-5 h-5 text-slate-300 group-hover:text-black transition" />
          </button>
        </div>
